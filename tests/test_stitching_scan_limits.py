...
class StitchingScanLimitTest(unittest.TestCase):
    def test_small_scan_uses_saved_tile_overlap_stitcher_for_final_mosaic(self):
        events = []
        ...
        plan = SimpleNamespace(
            ...
            cols=2,
            tiles=[
                TilePoint(row=0, col=0, x=0, y=0, z=0),
                TilePoint(row=0, col=1, x=8, y=0, z=0),
            ],
        )
        store = Mock(path=Path("/tmp/session"))
        store.save_tile.side_effect = lambda _frame, record: record
        final_mosaic = Path("/tmp/session/stitched_mosaic.png")
        ...
        with (
            ...
            patch("gui_app.stitch_session_by_metadata", return_value=final_mosaic) as offline_stitch,
        ):
        ...
        offline_stitch.assert_called_once_with(store.path)
        self.assertIn(("stitch_done", (store.path, final_mosaic)), events)
...
class StitchingCornerAutofocusTest(unittest.TestCase):
    ...
    def test_scan_worker_full_auto_focuses_three_by_three_probe_grid_for_plane(self):
        ...
        with (
            ...
            patch("gui_app.stitch_session_by_metadata", return_value=Path("/tmp/session/stitched_mosaic.png")),
        ):
            ProbeStationApp._stitching_scan_worker(
                fake_app,
                corners=corners,
